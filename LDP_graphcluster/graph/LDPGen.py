...
class LDPGen:
    def __init__(self, graph, num_clusters, epsilon):
        self.graph = graph
    ...
    def calculate_degree_vectors(self):
        ...
        for node in self.graph.nodes():
            ...
            for i, cluster in enumerate(self.clusters):
                degree_vector[i] = sum(1 for neighbor in self.graph.neighbors(node) if neighbor in cluster) + np.random.laplace(loc=0.0, scale=float((1/self.epsilon)))
    ...
    # def get_community_partition(self):
    #     partition = {}
    #     for cluster_idx, cluster_nodes in enumerate(self.clusters):
    #         for node in cluster_nodes:
    #             partition[node] = cluster_idx
    #     return partition
    def get_community_partition(self):
        partition = {}
        for cluster_idx, cluster_nodes in enumerate(self.clusters):
        ...
        sorted_partition = {node: partition[node] for node in sorted(partition.keys())}
...
if __name__ == '__main__':
    ...
    graph_clustering = LDPGen(G, num_clusters, epsilon)
